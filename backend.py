...
COMPANY_URLS = [
    "https://sozhaa.ai/",
    "https://sozhaa.ai/about",
    "https://sozhaa.ai/services",
    "https://sozhaa.ai/contact",
]
...
def fetch_snippets(urls, chars=1500):
    snippets = []
    headers = {"User-Agent": "SozhaaBot/1.0 (+https://sozhaa.ai)"}
    for u in urls:
        try:
...
def build_system_prompt(snippets):
    ...
    return (
        ...
        "(sozhaa.ai). Answer only about the company, services, pages and contact information. "
...
def build_html_email(user_details, service, transcript):
    header = f"""
    <h2>Sozhaa AI — Chat Transcript</h2>
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa AI 🚀<br>Our team will contact you soon."
        ...
        if payload.user_details.get("email"):
            thank_you_html = html + "<br><br><p>🙏 Thank you for chatting with Sozhaa AI. Our team will connect with you soon.</p>"
            send_email_with_attachment(
                payload.user_details["email"],
                "Sozhaa AI — Chat Summary",
                thank_you_html,
                TRANSCRIPT_EXCEL
            )
    ...
    if "support" in user_msg.lower() or "contact" in user_msg.lower():
        assistant_text = "Please contact us at our official email address: service@sozhaa.ai. Alternatively, a representative from our company will reach out to you shortly. 🚀 Our team will contact you soon."
        ...
        def support_alert():
            ...
            """
            send_email_with_attachment(COMPANY_EMAIL, "⚠️ Sozhaa AI — Support Request", alert_html)
            if payload.user_details.get("email"):
                send_email_with_attachment(
                    payload.user_details["email"],
                    "Sozhaa AI — Support Request Received",
    ...
    def save_and_email():
        ...
        if payload.user_details.get("email"):
            send_email_with_attachment(payload.user_details["email"], "Sozhaa AI — Your Chat Transcript", html, TRANSCRIPT_EXCEL)
